...
d_effect$Sol_type <- ifelse(d_effect$Solutions %in% c("Albedo", "Alkalinity glob.", "Cloud", "Hybrids", "Productivity", "Renewables", "Vegetation glob."), "global", "local")

# compute global feasability per solution
d_feas <- dst %>% 
  # focus on feasability
  filter(Criteria %in% c("Readiness", "Lead time until full pot. effectiv.")) %>% 
  # average scores
  group_by(Solutions) %>%
...
pv12 <- ggplot(pca_v12, aes(alpha=cos2)) +
  ...
  scale_alpha(limits=c(0,0.9), range = c(0.5,1)) +
...
pi12 <- ggplot(pca_i12, aes(Dim.1, Dim.2, alpha=cos2, colour=colour)) +
  geom_point(shape=16) + geom_text_repel(aes(label=label), size=3, show_guide=FALSE) +
...
library("gridExtra")
p <- grid.arrange(
  pv12 + paper_theme(12) + guides(alpha="none") + scale_y_continuous(breaks=0, expand=c(0.05,0.45)) + annotation_custom(grid::textGrob("A"), xmin=-1.6, xmax=-1.6, ymin=1.6, ymax=1.6),
  pi12 + paper_theme(12) + labs(alpha="Repr.", y=NULL) + annotation_custom(grid::textGrob("B"), xmin=5, xmax=5, ymin=3.8, ymax=3.8),
  nrow=1
)
# output at x2 the final size
# NB: font size was 12pt => 6pt at final size
ggsave(p, filename=paste0("fig_pca_", Sys.Date(), ".pdf"), width=4.75*2, units="in")
ggsave(p, filename=paste0("fig_pca_", Sys.Date(), ".png"), width=4.75*2, units="in")
```
