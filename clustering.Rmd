...
d_feas <- dst %>% filter(Criteria %in% c("Readiness", "Lead time until full pot. effectiv.")) %>% group_by(Solutions) %>% summarise(`Overall feasability`=mean(Score, na.rm=T))
...
dd$Solutions <- factor(dd$Criteria, levels=c(unique(dsst$Criteria), levels(dst$Criteria)))
