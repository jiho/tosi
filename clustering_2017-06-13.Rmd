...
d <- dst %>%
  mutate(
    ...
    Color=ifelse(is.na(Score), "grey", brewer_map(Score0, name="RgYlGn")) # red=0, green=5; alternatives RdYlBu (PRGn)
  ) %>%
...
```{r fig.width=4.75*2, warning=F, message=F}
...
library("gridExtra")
grid.arrange(
  pv12 + paper_theme(12) + guides(alpha="none") + scale_y_continuous(breaks=0, expand=c(0.05,0.45)),
  pi12 + paper_theme(12) + labs(alpha="Repr.", y=NULL),
  nrow=1
)
# output at x2 the final size
# NB: font size was 12pt => 6pt at final size
foo <- dev.print(device=pdf, file="fig_pca.pdf", width=4.75*2)
```
...
The first two principal components represented here explain 64% of the variance in the attributes of solutions. Attributes or solutions that are not well represented in this space are made semi-transparent (representativity = "Repr." varies between 0 and 1).  
The scores of attributes were averaged across ecosystems and ecosystem services.*

The first principal component is driven notably by governability, readiness, co-benefits and, to a lesser extent, the duration of the effect and the lack of unintended effects (Fig. Xa). Therefore, in first approximation, it scores "feasibility" or "practicality" [JOI: probablement choisir un des deux termes]. By definition of the PCA, solutions differ most in these practicality attributes (>40% of the variance). In particular, most solutions effective to reduce warming and its impacts are not practical. The second principal component is the direction towards which most arrows point and can be generally interpreted as "efficacy". A number of attributes (effectiveness to moderate sea level rise, lead time until full effectiveness, and cost effectiveness) were not immediately useful to discriminate solutions.

Three groups of solutions emerge (Fig. Xb). Measures in green (renewable energy, alkalinity addition at global scale, increased albedo and hybrid measures) are effective to reduce warming, acidification, and their impacts but, as explained above, score low in practicality. In contrast, measures in red have low effectiveness to reduce warming and its impacts and moderate effectiveness to reduce ocean acidification and its impact. They are, however, technically ready, have significant co-benefits, few dis-benefits and can also help reduce the impact of other drivers of climate change. This group comprises almost all local measures. Finally, solutions in the blue group do not score well in any of the attributes assessed. It comprises alkalinity addition implemented at local scale, evolution, productivity and marine cloud brightening, the later faring particularly poorly for moderation of ocean acidification.

### Methods

To get a synthetic view of the attributes of the various ocean-based solutions, they are summarised through a Principal Component Analysis (PCA) and grouped through hierarchical ascending clustering (HAC).

The Euclidean distance between the raw scores of all solutions for all attributes is computed, while PCA usually uses scaled values (centred on the mean, divided by the variance). Here, using the raw scores means that (i) the value we, as humans, place on an increase of 1 in the score is the same for all attributes and at any point in the scoring scale (the difference between 1 and 2 in readiness has the same value as the difference between 4 and 5 in the moderation of warming); (ii) attributes for which the scores vary little among solutions have low weight in the analysis. This Euclidian distance matrix is then used in the PCA. The quality of the representation of the attributes and solutions on each the principal components is quantified by their squared cosine. The squared cosines for principal components 1 and 2 used in Fig. X are summed to get the global representativity in the plane they define ("Repr." in the legend).

The relevance of the principal components to discriminate solutions is judged from their associated eigenvalues, compared to the mean eigenvalue (Kaiser-Guttman criterion) and a broken-stick model. Four principal components are retained and used for clustering. The clustering algorithm uses Euclidean distance and Ward's aggregation method, in order to highlight synoptic groups. The number of groups is defined based on the aspect of the resulting tree, which unambiguously separates three main groups before a succession of short branches in each group.

The analysis is performed on the full dataset and on a reduced dataset in which attributes are averaged per ecosystem and ecosystem services. The results are very similar and only the simpler version is kept for clarity.

All analyses are performed in R 3.4 with packages FactoMineR 1.36 for PCA, tidyverse 1.1.1 for data manipulation, and ggplot2 2.2.1 for graphics.
