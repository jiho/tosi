...
d <- dst %>%
  mutate(
    ...
    Color=ifelse(is.na(Score), "grey", brewer_map(Score0, name="RdYlGn")) # red=0, green=5; alternatives RdYlBu (PRGn)
  ) %>%
...
d_global <- filter(d, Criteria %in% levels(d$Criteria)[1:nlevels(t1$Criteria)])
chordDiagram(d_global,
  # map precomputed color to lines
  col=d$Color,
  order=c(levels(d$Criteria), levels(d$Solutions)),
  ...
  grid.col=rep(
    c("grey0", "grey80"),
    times=c(nlevels(t1$Criteria), nlevels(d$Solutions))
  ),
  # remove axis around plot
  annotationTrack=c("name", "grid"),
  # put green lines one top
...
d_local <- filter(d, Criteria %in% levels(d$Criteria)[(nlevels(t1$Criteria)+1):nlevels(d$Criteria)])
chordDiagram(d_local,
  # map precomputed color to lines
  col=d$Color,
  order=c(levels(d$Criteria), levels(d$Solutions)),
  # color grid according to type of data: criterion or solution
  grid.col=rep(
    c("grey0", "grey80"),
    times=c(nlevels(t3$Criteria), nlevels(d$Solutions))
  ),
  # remove axis around plot
  annotationTrack=c("name", "grid"),
  # put green lines one top
...
alluvial(
  # draw lines, ordered by score in each solution
  d_global[,2:1], freq=1,
  ordering=list(with(d_global, order(Solutions, Score)), NULL),
  # map color
  col=d_global$Color,
  # cleaner aspect
  border=NA, alpha=0.9, xw=0.2, cw=0.12, blocks=F, gap.width=0.15, cex=0.8,
...
alluvial(
  # draw lines, ordered by score in each solution
  d_local[,2:1], freq=1,
  ordering=list(with(d_local, order(Solutions, Score)), NULL),
  # map color
  col=d_local$Color,
  # cleaner aspect
  border=NA, alpha=0.9, xw=0.2, cw=0.12, blocks=F, gap.width=0.15, cex=0.8,
